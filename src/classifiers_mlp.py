...
def create_deep_early_fusion_model(text_input_size, image_input_size, output_size, hidden=[1024, 512, 256, 128], p=0.1):
    if text_input_size is None and image_input_size is None:
        raise ValueError("At least one of text_input_size and image_input_size must be provided.")
    ...
    for h in hidden:
        x = Dense(h, activation='relu')(x)
        x = BatchNormalization()(x)  # 🔥 Agregado clave
        x = Dropout(p)(x)
...
def train_mlp(train_loader, test_loader, text_input_size, image_input_size, output_size,
              num_epochs=50, report=False, lr=0.001, set_weights=True, adam=False, p=0.0,
              seed=1, patience=40, save_results=True, train_model=True, test_mlp_model=True,
              hidden=[128]):
    """
    ...
    model = create_deep_early_fusion_model(text_input_size, image_input_size, output_size, hidden=hidden, p=p)
